...
if chapter == "예상 효율 시뮬레이션":
    ...
    # 비교 대상 직업군 선택
        ...
        # 빈 공간 채우기
        st.markdown("---")
        st.markdown("이 시뮬레이션의 결과는 실제와 다를 수 있습니다. 참고용으로 사용해 주세요.")
    
    analysis = st.button("예상 효율 분석하기", use_container_width=True)
    
    if analysis:
        # 기준 직업과 대상 직업이 같은지 확인
    ...
    st.write("""
             \n분석에 사용된 **전사, 마법사, 궁수, 도적, 해적** 5개 분류 및 세부 직업 목록
             """)
    ...
    st.write("""
             ...
             \n▷ 본 분석에서 직업에 따른 전투력 차이가 통계적으로 유의하지 않을 경우, 직업 간 밸런스가 비교적 잘 유지되고 있다고 해석.
             """)

    st.subheader("1.2. 데이터 수집")
    st.write("""
             #### 데이터 수집
             \n넥슨 open api에서 메이플스토리 데이터를 수집했.
             \n#### 표본
    ...
    st.markdown("분석 대상 데이터를 기반으로 한 **큰 직업 분류**와 이에 속하는 **세부 직업** 목록")
    ...
    st.write("""#### 결과 해석
             \n- R-squared**: 0.723
             \n    ▷ 종속변수의 변동 중 약 72%가 이 모형의 독립변수로 설명됨
             """)

    st.write("""
             \n- 주스탯: 계수 0.217, p < 0.001
             \n    ▷ 전투력에 가장 강한 영향
             \n- 보스 몬스터 데미지: 계수 0.27, p < 0.001
             \n    ▷ 전투력 산식에 꽤 직접적으로 반영
             \n- 크리티컬 데미지: 계수 0.080, p = 0.001
             \n    ▷ 영향이 보스 몬스터 데미지나 주스탯보다는 작음
             \n- 방어율 무시: 계수 0.062, p < 0.034
             \n    ▷ 효과는 작지만 유의한 수준
             \n즉, 전투력은 **스탯 투자 수준**의 영향이 크다
             """)

    st.write("#### 직업 분류별 전투력 영향 (회귀계수)")
    st.markdown("계수가 높을수록 기준 그룹(궁수) 대비 효율이 좋음")
    ...
    st.write("""#### 궁수 대비 전투력(log) 차이
             \n- 도적: +0.152
             \n     ▷ p-value가 0.072이므로 통계적으로 유의미한 차이는 아님
             \n- 마법사: +0.243
             \n    ▷ p-value가 0.002이므로 통계적으로 유의미한 차이
             \n- 전사: +0.143
             \n    ▷ p-value가 0.073이므로 통계적으로 유의미한 차이는 아님
             \n- 해적: +0.337
             \n    ▷ p-value가 0.001이므로 통계적으로 매우 유의미한 차이  
             \n즉, 직업 간 차이는 일부 존재. 특히 해적, 마법사가 궁수보다 전투력이 높다는 것이 확인되었음.           
             """)
    ...
    st.write("""예측값($\hat{Y}$)에 따른 잔차($Y - \hat{Y}$)의 분포
             """)
    ...
    st.write("""
             잔차가 예측값 전반에 걸쳐 **무작위적으로 분포**
             \n▷ 선형성과 등분상성에 크게 문제 없음
             \n▷ 선형 회귀모형의 기본 가정이 크게 위배되지 않는 것으로 판단
             """)


    st.write("#### 다중공선성 진단")
    st.write("주요 통제 변수(스펙) 간의 다중공선성을 확인")
    ...
    st.write("다중공선성이 10을 넘지 않으므로 큰 문제는 없음")
    ...
    st.write("""
             1. 방어율 무시와 보스 몬스터 데미지는 보스 콘텐츠를 중심으로 함께 강화되는 스탯이기 때문에 높은 상관관계를 보임
             \n2. 주스탯은 전반적인 성장 수준을 반영하는 기본 스탯이므로 다른 스탯들과 양의 상관관계를 보임
             \n3. 크리티컬 데미지는 다른 스탯들 간의 관계보다는 독립적인 편
             """)

elif section == "3. 결론":
    st.header("3. 결론")
    st.write("""### 핵심 결과 요약
             \n회귀분석에서 직업 더미 변수들은 대부분 **통계적으로 유의하지 않거나**, 일부 유의하더라도 그 영향력이 주스탯이나 주요 스펙 변수에 비해 **상대적으로 크지 않은 것**으로 나타남
             \n▷ 직업 자체가 전투력을 결정짓는 핵심 요인이라고 보기는 어려움
             \n스펙 변수: 전투력과 강한 양의 관계
             \n▷ 전투력이 직업의 고유한 성능보다는 **캐릭터가 얼마나 투자되고 성장했는지를 반영하는 지표**라는 점과도 일치             
             """)
    st.write("""### 결론
             \n일부 직업은 기준 직업군(궁수)과 비교했을 때 전투력에서 통계적으로 유의한 차이를 보였지만, 직업군 전반에 걸친 일관된 효과는 확인되지 않았음
             \n**귀무가설 기각 불가**: 모든 직업군에서 통계적으로 유의한 차이가 나타난 게 아니므로, 직업 자체가 전투력을 결정짓는 지배적인 요인이라고 단정하기는 어려움
             \n전투력 지표 상에서는 직업 간 밸런스가 비교적 유지되고 있는 것으로 해석됨
             """)
